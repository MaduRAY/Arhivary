...
void Start ( bool End, enum, int x, int y, int Fx, int fY, int score ) {	    //Функция определяет координаты поля, фруков и задает контроьную переменную End
    End = false;
    enum vec {
	   Stop = 0, Up, Down, Right, Left
	   };
...
void Map ( int: tX[], tY[], score, wid, bool T ) {		    //Функция отвечает за заполнение поля (фрукты, змейку, хвост, количество очков)
...
void Input ( enum vec ) {	    //Обработка ввода/направения движения
    enum vec {
	   Stop = 0, Up, Down, Right, Left
	   };
    vec way
...
void Mechanics ( enum vec, int pX = tX[0], pY = tY[0], p2X, p2Y, wid, hei, Fx, Fy ) {		   //Функция описывает процесс изменений на поле
    enum vec {
	   Stop = 0, Up, Down, Right, Left
	   };
    vec way
    ...
    if ( x >= wid - 1 ) {   //Механика прохода змейкой через стенки
	   x = 0;
	   } else if ( x < 0 ) {
		  x = wid - 2;
		  }
